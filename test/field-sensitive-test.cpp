...
static void pointsTo(Module &M, const ToCheck &toCheck) {
...
#ifdef DEBUG
  for (ptr::PointsToSets::const_iterator I = PS.begin(), E = PS.end(); I != E;
       ++I) {
    const Ptr &ptr = I->first;
    errs() << "OFF=" << ptr.second;
    ptr.first->dump();
    const PTSet &p = I->second;
    for (PTSet::const_iterator II = p.begin(), EE = p.end(); II != EE; ++II) {
      const Ptee &ptee = *II;
  ...
  errs() << "======\n";
#endif
  for (ToCheck::const_iterator I = toCheck.begin(), E = toCheck.end(); I != E;
       ++I) {
    const Ptr &ptr = I->first;
    ...
    bool found = false;
    for (PTSet::const_iterator II = S.begin(), EE = S.end(); II != EE; ++II) {
      const Ptee &ptee2 = *II;
      if (ptee1 == ptee2) {
        found = true;
        break;
    ...
    if (!found) {
      errs() << "Cannot find pointee for OFF=" << ptr.second;
      ptr.first->dump();
...
static Value *call_malloc(ToCheck &toCheck, BasicBlock *entry,
  ...
  Value *ret = CallInst::Create(
      xmalloc, ArrayRef<Value *>(ConstantInt::get(Type::getInt64Ty(C), size)),
      "", entry);
...
static std::unique_ptr<Module> build(LLVMContext &C, ToCheck &toCheck) {
  ...
  FunctionType *mallocTy = FunctionType::get(
      Type::getInt8PtrTy(C), ArrayRef<Type *>(Type::getInt64Ty(C)), false);
  Function *xmalloc =
      Function::Create(mallocTy, GlobalValue::ExternalLinkage, "malloc", M);

  GlobalVariable *glob =
      new GlobalVariable(*M, Type::getInt32Ty(C), false,
                         GlobalValue::InternalLinkage, 0, "global");
  (void)glob;
  ...
  Value *bcast =
      new BitCastInst(malloc48, PointerType::getUnqual(xstruct), "", entry);
  ...
  Value *gep = GetElementPtrInst::CreateInBounds(bcast, gepIdx, "", entry);
  ...
  Value *pi = new AllocaInst(PointerType::getUnqual(xstruct), 0, "", entry);
  ...
  gep = GetElementPtrInst::CreateInBounds(load, gepIdx, "", entry);
