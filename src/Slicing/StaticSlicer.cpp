...
StaticSlicer::StaticSlicer(ModulePass *MP, Module &M,
    ...
    : MP(MP), module(M), slicers(), initFuns(), funcsToCalls(), callsToFuncs() {
...
static RegisterPass<Slicer> X("slice-inter",
                              "Slices the code interprocedurally");
char Slicer::ID;

bool Slicer::runOnModule(Module &M) {
  // module => points to sets
  ptr::PointsToSets PS;
  {
  ...
  // module, points to sets => call graph
