...
static GlobalVariable *getAiVar(Function &F, const CallInst *CI) {
  const ConstantExpr *GEP = dyn_cast<const ConstantExpr>(CI->getOperand(1));
  ...
  delete cstr;
  glob->setInitializer(
      ConstantInt::get(TypeBuilder<int, false>::get(F.getContext()), 0));
  return glob;
}
...
bool Prepare::handleAsm(Function &F, CallInst *CI) {
  ...
  if ((ASM.empty() && !CI->getNumArgOperands()) || /* a barrier */
      !ASM.compare(0, 6, "1:\tud2") || !ASM.compare("lfence") ||
      !ASM.compare("mfence") || !ASM.compare("sfence")) {
    /*    errs() << ASM << " (" << F.getName() << "): " << ASM.empty() << " " <<
    ...
    ReplaceInstWithInst(
        CI, CastInst::CreatePointerCast(CI->getArgOperand(0), CI->getType()));
    return true;
    ...
    if (param) {
      ...
      if (paramVal == 0) { /* current */
        ReplaceInstWithInst(CI, new LoadInst(M->getOrInsertGlobal(
                                    "__ai_current_singleton", CI->getType())));
        return true;
      } else { /* others, let's fake it with global var */
        GlobalVariable *GV = dyn_cast<GlobalVariable>(M->getOrInsertGlobal(
            "__ai_pda_" + paramVal.toString(10, false), CI->getType()));
        GV->setInitializer(Constant::getNullValue(CI->getType()));
        if (CI->getType()->isPointerTy())
          errs() << "Warn ptr type => we set it to point to nullptr\n";
...
void Prepare::makeNop(Function *F) {
  F->deleteBody();
  BasicBlock *BB =
      BasicBlock::Create(F->getContext(), "entry", F); // F.push_back(BB)
...
void Prepare::deleteAsmBodies(llvm::Module &M) {
  static const char *toDelete[] = {
      ...
      "___arch__swab32",
      "___arch__swab64" // generates false positives in stats
  };
  static const char *_makeNop[] = {"pagefault_disable",
  ...
  for (i = 0; i < ARRAY_SIZE(toDelete); i++) {
    ...
    if (F)
      F->deleteBody(); // convert function definition to declaration
  }
...
bool Prepare::addInitFun(const callgraph::Callgraph &CG,
                         SmallVector<Constant *, 10> &initFns, Type *ETy,
                         Function &F, bool starting) {
  if (F.isDeclaration())
    return false;
  if (starting) {
...
void Prepare::findInitFuns(Module &M, const ptr::PointsToSets &PS) {
  ...
  new GlobalVariable(M, ATy, true, GlobalVariable::InternalLinkage,
                     ConstantArray::get(ATy, initFns), "__ai_init_functions");
}
