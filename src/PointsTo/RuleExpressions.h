...
template <typename SubExpr> struct RuleUnaryExpression : public Expression {
  typedef SubExpr SubExpression;

  explicit RuleUnaryExpression(SubExpression const sub) : arg(sub) {}

  SubExpression getArgument() const { return arg; }
...
struct RuleBinaryExpression : public Expression {
  ...
  SubExpression1 getArgument1() const { return arg1; }

  SubExpression2 getArgument2() const { return arg2; }
...
template <typename MemLoc> struct ALLOC : public RuleUnaryExpression<MemLoc> {
  ALLOC(MemLoc const ml) : RuleUnaryExpression<MemLoc>(ml) {}
};

template <typename MemLoc> struct DEALLOC : public RuleUnaryExpression<MemLoc> {
  DEALLOC(MemLoc const ml) : RuleUnaryExpression<MemLoc>(ml) {}
};

template <typename SubExpr> struct GEP : public RuleUnaryExpression<SubExpr> {
  GEP(SubExpr const sub) : RuleUnaryExpression<SubExpr>(sub) {}
};

template <typename MemLoc> struct NULLPTR : public RuleUnaryExpression<MemLoc> {
...
template <typename ExprSort> struct RuleExpression {
  typedef ExprSort Sort;

  explicit RuleExpression(Sort const &s) : sort(s) {}
...
struct RuleCode {
  ...
  RuleCode() : type(RCT_UNKNOWN) {}

  RuleCode(ASSIGNMENT<VARIABLE<MemoryLocation>, ALLOC<MemoryLocation>> const &E)
      : type(RCT_VAR_ASGN_ALLOC), lvalue(E.getArgument1().getArgument()),
        rvalue(E.getArgument2().getArgument()) {}

  RuleCode(
      ASSIGNMENT<VARIABLE<MemoryLocation>, NULLPTR<MemoryLocation>> const &E)
      : type(RCT_VAR_ASGN_NULL), lvalue(E.getArgument1().getArgument()),
        rvalue(E.getArgument2().getArgument()) {}

  RuleCode(
      ASSIGNMENT<VARIABLE<MemoryLocation>, VARIABLE<MemoryLocation>> const &E)
      : type(RCT_VAR_ASGN_VAR), lvalue(E.getArgument1().getArgument()),
        rvalue(E.getArgument2().getArgument()) {}

  RuleCode(ASSIGNMENT<VARIABLE<MemoryLocation>,
                      GEP<VARIABLE<MemoryLocation>>> const &E)
      : type(RCT_VAR_ASGN_GEP), lvalue(E.getArgument1().getArgument()),
        rvalue(E.getArgument2().getArgument().getArgument()) {}

  RuleCode(ASSIGNMENT<VARIABLE<MemoryLocation>,
                      REFERENCE<VARIABLE<MemoryLocation>>> const &E)
      : type(RCT_VAR_ASGN_REF_VAR), lvalue(E.getArgument1().getArgument()),
        rvalue(E.getArgument2().getArgument().getArgument()) {}

  RuleCode(ASSIGNMENT<VARIABLE<MemoryLocation>,
                      DEREFERENCE<VARIABLE<MemoryLocation>>> const &E)
      : type(RCT_VAR_ASGN_DREF_VAR), lvalue(E.getArgument1().getArgument()),
        rvalue(E.getArgument2().getArgument().getArgument()) {}
  ...
  RuleCode(DEALLOC<MemoryLocation> const &E)
      : type(RCT_DEALLOC), lvalue(E.getArgument()), rvalue() {}
