...
void CallMaps::buildCallMaps(const Module &M) {
  for (Module::const_iterator f = M.begin(); f != M.end(); ++f) {
    ...
    for (const_inst_iterator
             i = inst_begin(f.operator const llvm::Function *()),
             E = inst_end(f.operator const llvm::Function *());
         i != E; ++i) {
...
ProgramStructure::ProgramStructure(const Module &M) : M(M) {
  ...
  for (const_iterator I = getContainer().begin(), E = getContainer().end();
    ...
    if (rc.getLvalue()) { // add by jiangg 2015.11.3
      rc.getLvalue()->dump();
    }
    errs() << "\tR=";
    if (rc.getRvalue()) {
